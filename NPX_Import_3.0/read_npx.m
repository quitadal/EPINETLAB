...
% if  nargin < 1
%     [filename_bin, filepath_bin] = uigetfile('*.bin', 'Choose a BIN file');
% % else
...
% % end;
%
% for i= 1:length(filename_bin)-4
%     name_file(i)=filename_bin(i);
% end

% filename_npx=sprintf('%s%s', name_file, '.npx');
% filepath_npx=filepath_bin;
%
% fullFileName_bin = sprintf('%s%s', filepath_bin, filename_bin);
% fullFileName_npx = sprintf('%s%s', filepath_npx, filename_npx);
%
