%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PLOT DETECTION RESULTS WITH  BIPOLAR AND GRID CHANNELS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
figSearch=  findobj('type','figure');
nFig = length(figSearch);
...
for n= 1:nFiles
    ...
    set(tt,'FontWeight', 'bold', 'FontSize', 11, 'Color', 'r', 'Position', ttPos + [0 -ttPos(2)/8 0] )
    box off
...
for s= 1:length(SingleMontage{1,1})
    ...
    % chsetIdxfull= cell2mat(cellfun(@(x,y,n) strncmpi(x,y,strLen), {monCh}, BipMontage,  'UniformOutput', 0));
    ...
    if strcmp(SingleMontage{1,1}{s,1}, 'Gd') || strcmp(SingleMontage{1,1}{s,1}, 'Gr') || strcmp(SingleMontage{1,1}{s,1}, 'MG')|| strcmp(SingleMontage{1,1}{s,1}, 'LG')
        ss=1; rr= 1; tt= 1; vv= 1;
        for l= 1: length(chsetIdx)
            if  colorIdx(l,1)==1 && colorIdx(l,2)==1 && colorIdx(l,3)==1
                if l<= 8
                    rectangle('Position',[ss 4 0.5 0.5], 'FaceColor', colorIdx(l,:), 'EdgeColor', 'k');
                    if TotChMon(chsetIdx(l))~=0
                        text(ss, 4.6, [BipMontage{1,1}{chsetIdx(l),1} '[' sprintf('%4.2f', TotChMon(chsetIdx(l))) '%]'], 'FontWeight', 'bold', 'FontSize', 9);
                    else
                        text(ss, 4.6, BipMontage{1,1}{chsetIdx(l),1}, 'FontWeight', 'bold', 'FontSize', 9);
                    end
                    ss= ss+1;
                elseif l>8 && l<=16
                    rectangle('Position',[rr 3 0.5 0.5], 'FaceColor', colorIdx(l,:), 'EdgeColor', 'k');
                    if TotChMon(chsetIdx(l))~=0
                        text(rr, 3.6, [BipMontage{1,1}{chsetIdx(l),1} '[' sprintf('%4.2f', TotChMon(chsetIdx(l))) '%]'], 'FontWeight', 'bold', 'FontSize', 9);
                    else
                        text(rr, 3.6, BipMontage{1,1}{chsetIdx(l),1}, 'FontWeight', 'bold', 'FontSize', 9);
                    end
                        
                    rr=rr+1;
                elseif l>16 && l<=24
                    rectangle('Position',[tt 2 0.5 0.5], 'FaceColor', colorIdx(l,:), 'EdgeColor', 'k');
                    if TotChMon(chsetIdx(l))~=0
                        text(tt, 2.6, [BipMontage{1,1}{chsetIdx(l),1} '[' sprintf('%4.2f', TotChMon(chsetIdx(l))) '%]'], 'FontWeight', 'bold', 'FontSize', 9);
                    else
                        text(tt, 2.6, BipMontage{1,1}{chsetIdx(l),1}, 'FontWeight', 'bold', 'FontSize', 9);
                    end
                    tt=tt+1;
                elseif l>24
                    rectangle('Position',[vv 1 0.5 0.5], 'FaceColor', colorIdx(l,:), 'EdgeColor', 'k');
                    if TotChMon(chsetIdx(l))~=0
                        text(vv, 1.6, [BipMontage{1,1}{chsetIdx(l),1} '[' sprintf('%4.2f', TotChMon(chsetIdx(l))) '%]'], 'FontWeight', 'bold', 'FontSize', 9);
                    else
                        text(vv, 1.6, BipMontage{1,1}{chsetIdx(l),1}, 'FontWeight', 'bold', 'FontSize', 9);
                    end
                    vv= vv+1;
                end
                ...
                if l<= 8
                    rectangle('Position',[ss 4 0.5 0.5], 'FaceColor', colorIdx(l,:), 'EdgeColor', 'w');
                    if TotChMon(chsetIdx(l))~=0
                        text(ss, 4.6, [BipMontage{1,1}{chsetIdx(l),1} '[' sprintf('%4.2f', TotChMon(chsetIdx(l))) '%]'], 'FontWeight', 'bold', 'FontSize', 9);
                    else
                        text(ss, 4.6, BipMontage{1,1}{chsetIdx(l),1}, 'FontWeight', 'bold', 'FontSize', 9);
                    ...
                    ss= ss+1;
                elseif l>8 && l<=16
                    rectangle('Position',[rr 3 0.5 0.5], 'FaceColor', colorIdx(l,:), 'EdgeColor', 'w');
                    if TotChMon(chsetIdx(l))~=0
                        text(rr, 3.6, [BipMontage{1,1}{chsetIdx(l),1} '[' sprintf('%4.2f', TotChMon(chsetIdx(l))) '%]'], 'FontWeight', 'bold', 'FontSize', 9);
                    else
                        text(rr, 3.6, BipMontage{1,1}{chsetIdx(l),1}, 'FontWeight', 'bold', 'FontSize', 9);
                    ...
                    rr= rr+1;
                elseif l>16 && l<=24
                    rectangle('Position',[tt 2 0.5 0.5], 'FaceColor', colorIdx(l,:), 'EdgeColor', 'w');
                    if TotChMon(chsetIdx(l))~=0
                        text(tt, 2.6, [BipMontage{1,1}{chsetIdx(l),1} '[' sprintf('%4.2f', TotChMon(chsetIdx(l))) '%]'], 'FontWeight', 'bold', 'FontSize', 9);
                    else
                        text(tt, 2.6, BipMontage{1,1}{chsetIdx(l),1}, 'FontWeight', 'bold', 'FontSize', 9);
                    ...
                    tt= tt+1;
                elseif l>24
                    rectangle('Position',[vv 1 0.5 0.5], 'FaceColor', colorIdx(l,:), 'EdgeColor', 'w');
                    if TotChMon(chsetIdx(l))~=0
                        text(vv, 1.6, [BipMontage{1,1}{chsetIdx(l),1} '[' sprintf('%4.2f', TotChMon(chsetIdx(l))) '%]'], 'FontWeight', 'bold', 'FontSize', 9);
                    else
                        text(vv, 1.6, BipMontage{1,1}{chsetIdx(l),1}, 'FontWeight', 'bold', 'FontSize', 9);
        ...
        ylim([0 5])
        text(ax(s).XLim(1)-(ax(s).XLim(2)-ax(s).XLim(1))/10, 2.5 , 'Grid [4x8]', 'FontWeight', 'bold', 'FontSize', 12);
